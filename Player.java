...
public class Player {
	...
	public void moveCreature(Player otherPlayer)
	{
		...
		//Bounds checking
		while(row >= board.getBoard().length || col >= board.getBoard()[0].length || row < 0 || col < 0 || board.getBoard()[row][col] == null)
		{
			System.out.println("Invalid coordinates given.");
			System.out.println("Enter the X and Y coordinate of the creature you'd like to move.");
			System.out.print("X: ");
		...
		if(board.getBoard()[row][col] != null)
		{
			...
			{
				removeFromPlayedCreatures(selected);
	...
	public void removeFromPlayedCreatures(Creature creature)
	{
		for(int i = 0; i < playedCreatures.size(); i++)
		{
			if(playedCreatures.get(i).equals(creature))
				playedCreatures.remove(i);
		}
	}
